...
export type GqlAppBuilderMiddleware = (
    ...
    | null
    | undefined
    | Promise<void | null | undefined | GqlAppBuilderMiddlewareCallback>;
...
export class GqlAppBuilder {
    ...
    static getMatchMiddlewares(
        ...
        if (
            middlewares &&
            typeName in middlewares &&
            fieldName in middlewares[typeName]
        ) {
            ...
            for (const typeName in module.resolvers) {
                if (module.resolvers[typeName] instanceof GraphQLScalarType) {
                    if (typeName in resolvers) {
                        throw Error(
                            'gqlAppBuilder build error. (already exist resolver type)',
                        );
                    }
                    ...
                    for (const fieldName in module.resolvers[typeName]) {
                        ...
                        middlewares.push(
                            ...GqlAppBuilder.getMatchMiddlewares(
                                '*',
                                '*',
                        ...
                        if (resolvers[typeName][fieldName]) {
                            throw Error(
                                'gqlAppBuilder build error. (already exist resolver field)',
                            );
                        }

                        if (typeName === 'Subscription') {
                            resolvers[typeName][fieldName] =
                                module.resolvers[typeName][fieldName];
                        } else {
                            resolvers[typeName][fieldName] = async (
                                ...
                                const injector = new InjectorWrapper(
                                    ctx.injector,
                                );
                                try {
                                    for (const middleware of middlewares) {
                                        const cb = await middleware(
                                            injector,
                                            parent,
                                            args,
                                            info,
                                    ...
                                    resolveData = module.resolvers[typeName][
                                        fieldName
                                    ](injector, parent, args, info);
                                } catch (error) {
                                    resolveError = error;
                                }
...
const gqlQueryLogger = createLogger<{
    ...
    consoleFormat: ({ level, message, subData, timestamp }) =>
        `${timestamp} [GQL_QUERY] ${level}: #${subData.reqKey}# ${message} \n${
            ...
            subData?.params && !isEmpty(subData.params)
                ? `=> (Params: ${JSON.stringify(subData.params)})`
                : ''
        }`,
});

const gqlFieldLogger = createLogger<{
    reqKey: string;
    type: string;
    fieldName: string;
}>('GQL_FIELD', {
    ...
    consoleFormat: ({ level, message, subData, timestamp }) =>
        `${timestamp} [GQL_FIELD] ${level}: #${subData.reqKey}# ${message}`,
});

export const logMiddleware: GqlAppBuilderMiddleware = (
    injector,
    parent,
    args,
    info,
    ...
    return async (resolveData, resolveError) => {
        if (resolveError) {
            const dbConnectionService =
                injector.resolve<DatabaseConnectionService>(
...
export const orderByIdArray = (
    arr: any[],
    idArr: ReadonlyArray<string | number>,
    getIdFn: (item: any) => string | number = (item) => item.id,
) => {
...
export const gqlAppBuilderContextMapper = async ({
    req,
    res,
}: {
    req: Request;
    ...
    injector
        .register<Request>(REQUEST, { useValue: req })
